...
### 🧭 备份策略中心 + 定时任务增强 + 恢复可靠性增强

- 新增「备份策略中心」：智能体记忆、完整 QwenPaw、全局设置、技能池、敏感密钥分组选择。
- 敏感密钥默认不备份；选择密钥时必须启用加密要求，未提供加密密码会拒绝备份。
- 新增计划任务中心：展示全局/每源计划、频率、下次运行估算、最近结果。
- 新增移动端「更多」隐藏式导航，底部栏保留高频入口。
- 新增恢复预检 API：检查归档完整性、目标路径白名单、目标/父目录可写、目录缺失创建能力、QwenPaw workspaces/技能/全局配置检测。
- 新增 QwenPaw 整体恢复 API：支持目标目录不存在时自动创建，执行前强制保护快照。
- 恢复中心增加「恢复预检」按钮，便于重装系统/重装 QwenPaw 后先做 dry-run 验证。

### 验证

- Node syntax check 覆盖前后端改动文件。
- 恢复预检/策略/调度 API 待生产同步后验证。
